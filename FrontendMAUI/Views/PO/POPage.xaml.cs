...
namespace FrontendMAUI.Views.PO;

public partial class POPage : ContentPage
{
    ...
    private async void POList_ItemTapped(object sender, ItemTappedEventArgs e)
    {
        if (e.Item is ArchivoDocumento item)
        {
            try
            {
                if (item.EsCarpeta)
                {
                    string rutaRelativa = item.Ruta.Replace("po/", "");
                ...
                else
                {
                    ...
                    var url = $"{Config.BackendUrl}/po/abrir?archivo={encodedRuta}";
    ...
    private async void CargarContenido(string ruta)
    {
        try
        {
            var contenido = await POService.ObtenerContenidoAsync(ruta);
            POList.ItemsSource = contenido;
    ...
    public static implicit operator View(POPage v)
    {
    ...
    private async void AñadirArchivoBtn_Clicked(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            string rutaActual = rutaStack.Count > 0 ? rutaStack.Peek() : "";
            string carpetaBackend = string.IsNullOrEmpty(rutaActual) ? "po" : Path.Combine("po", rutaActual).Replace("\\", "/");
    ...
    {
        var url = $"{Config.BackendUrl}/manuales/upload"; // Ajusta la URL base si es necesario
