// Views/ManualesMedicinas/ManualMedicinaPage.xaml.cs
...
namespace FrontendMAUI.Views.ManualesMedicinas;

// Página para explorar y gestionar manuales de medicinas (PDFs y carpetas)
public partial class ManualMedicinaPage : ContentPage
{
    // Pila para gestionar la navegación entre carpetas
    private Stack<string> rutaStack = new();
    private readonly HttpClient _http = new();
    // Indica si el usuario es administrador (controla la visibilidad de botones)
    public static bool EsAdmin { get; set; } = false;

    public ManualMedicinaPage()
    {
        InitializeComponent();
        CargarContenido("");
    ...
    private async void ListaMedicinas_ItemTapped(object sender, ItemTappedEventArgs e)
    {
        if (e.Item is ArchivoDocumento item)
        {
            try
            {
                if (item.EsCarpeta)
                {
                    string rutaRelativa = item.Ruta.Replace("manuales_medicinas/", "");
                ...
                else
                {
                    ...
                    var url = $"{Config.BackendUrl}/manuales_medicinas/abrir?archivo={encodedRuta}";
    ...
    private async void CargarContenido(string ruta)
    {
        try
        {
            var contenido = await ManualMedicinaService.ObtenerContenidoAsync(ruta);
            ListaMedicinas.ItemsSource = contenido;
    ...
    private async void ListaMedicinas_SelectionChanged(object sender, SelectionChangedEventArgs e)
    {
        if (e.CurrentSelection.FirstOrDefault() is ArchivoDocumento item)
        {
            ...
            ListaMedicinas.SelectedItem = null;
        }
    }
    ...
    private async void AñadirArchivoBtn_Clicked(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            string rutaActual = rutaStack.Count > 0 ? rutaStack.Peek() : "";
            string carpetaBackend = string.IsNullOrEmpty(rutaActual) ? "manuales_medicinas" : Path.Combine("manuales_medicinas", rutaActual).Replace("\\", "/");
