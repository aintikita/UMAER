// Views/ManualesEquipos/ManualEquipoPage.xaml.cs
...
namespace FrontendMAUI.Views.ManualesEquipos;

// Página para explorar y gestionar manuales de equipos (PDFs y carpetas)
public partial class ManualEquipoPage : ContentPage
{
    ...
    private async void ListaManuales_ItemTapped(object sender, ItemTappedEventArgs e)
    {
        if (e.Item is ArchivoDocumento item)
        {
            try
            {
                if (item.EsCarpeta)
                {
                    string rutaRelativa = item.Ruta.Replace("manuales_equipos/", "");
                ...
                else
                {
                    ...
                    var url = $"{Config.BackendUrl}/manuales_equipos/abrir?archivo={encodedRuta}";
    ...
    private async void CargarContenido(string ruta)
    {
        try
        {
            var contenido = await ManualEquipoService.ObtenerContenidoAsync(ruta);
            ListaEquipos.ItemsSource = contenido;
    ...
    private async void AñadirArchivoBtn_Clicked(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            string carpetaBackend = string.IsNullOrEmpty(rutaActual) ? "manuales_equipos" : Path.Combine("manuales_equipos", rutaActual).Replace("\\", "/");
